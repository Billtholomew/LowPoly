...
def get_triangulation(im, e_factor=0.33, sigma=0.33):

    edges = auto_canny(im, sigma)
    contours, _ = cv2.findContours(edges, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    epsilons = map(lambda x: e_factor * cv2.arcLength(x, True) / 100.0, contours)
...
def wrapper(in_name, out_name=None, show=False, new_size=None, debug=False):

    oim = cv2.imread(in_name, flags=cv2.CV_LOAD_IMAGE_COLOR)  # ensure we read it in as a color image
    if new_size is not None:
        scale = new_size / float(np.max(oim.shape[:2]))
        oim = cv2.resize(oim, (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC)

    e_factor = 0.01
    sigma = 0.33

    t = time.time()
    triangle_data = get_triangulation(oim, e_factor, sigma)
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        if len(sys.argv) == 3:
            outFile = sys.argv[2]
        wrapper(in_name=inFile, out_name=outFile, show=True, new_size=None, debug=True)
